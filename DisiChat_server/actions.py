...
class InfoForm(FormAction):
    ...
    def validate_num_phone(
        ...
        if self.is_int(value) and len(str(value)) == 10:
...
class ProblemForm(FormAction):
    ...
    def required_slots(tracker: Tracker) -> List[Text]:
        ...
        return ["problem","peoble_other","num_phone"]
    ...
    def slot_mappings(self) -> Dict[Text, Union[Dict, List[Dict]]]:
        ...
        return { "problem": self.from_text(intent=None), "peoble_other": self.from_text(intent=None),
        "num_phone": self.from_text(intent=None)
		}
		
    @staticmethod
    def is_long(string: Text) -> bool:
        """Check if a string is an integer"""
        if len(string)>9 and len(string)<11:
            return True
        else:
            return False		
    ...
    def validate_num_phone(
        ...
        if  (self.is_int(value) and self.is_long(value)) :
            return {"num_phone": value}
        else:
            dispatcher.utter_message(template="utter_wrong_num_phone")
            # validation failed, set slot to None
